...
import { beginRegister,beginLogin, duplicateUser,duplicateUserSuccess, fetchmenu, fetchmenusuccess, getroles, getrolesuccess, getuserbycode, getuserbycodesuccess, getusers, getuserssuccess, updateuserrole } from './User.action';
      ...
      exhaustMap((action) => {
        return this.service.UserRegistration(action.userdata).pipe(
          ...
          catchError((_error) =>
            of(
              showalert({
                message: 'Registeration Failed due to: ' + _error.message,
                resulttype: 'fail',
              })
            )
          )
...
// function fetchmenu(arg0: { userrole: string; }): any {
